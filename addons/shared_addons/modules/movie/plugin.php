 ...
 * @package  PyroCMS\Core\Modules\Movie\Plugins
 */
class Plugin_Movie extends Plugin
{
	...
	public $description = array(
		'en' => 'A plugin to display information such as movie categories and posts.',
		'br' => 'Um plugin para exibir informações como categorias e postagens do movie.',
            'fa' => 'یک پلاگین برای نمایش اطلاعاتی مانند مجموعه های بلاگ و پست ها',
        'fr' => 'Un plugin permettant d\'afficher des informations comme les catégories et articles du movie.'
	);
		...
		$info = array(
			'posts' => array(
				'description' => array(// a single sentence to explain the purpose of this method
					'en' => 'Display movie posts optionally filtering them by category.',
					'br' => 'Mostra postagens do movie opcionalmente filtrados por categoria.',
                    'fr' => 'Permet d\'afficher des articles de movie en les filtrants par catégorie.'
				),
			...
			'categories' => array(
				'description' => array(
					'en' => 'List movie categories.',
					'br' => 'Lista categorias do movie.',
                    'fr' => 'Lister les catégories du movie'
				),
				'single' => false,
				'double' => true,
			...
			'count_posts' => array(
				'description' => array(
					'en' => 'Count movie posts that meet the conditions specified.',
					'br' => 'Conta as postagens do movie que satisfazem as condições especificadas.',
                    'fr' => 'Permet de compter les articles de movie qui remplissent certaines conditions spécifiées.'
				),
				'single' => true,
				'double' => false,
				'variables' => '',
			...
			'tags' => array(
				'description' => array(
					'en' => 'Retrieve all tags that have been applied to movie posts.',
					'br' => 'Recupera todas as tags que foram aplicadas à postagens do movie.',
	 ...
	 * Movie List
	 *
	 * Creates a list of movie posts. Takes all of the parameters
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('movie_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movie_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movie_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movie_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movie_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('movie_categories', true).' ON '.$this->db->protect_identifiers('movie_categories.id', true).' = '.$this->db->protect_identifiers('movie.category_id', true);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'movie', 'movies', $loop);
	}

	/**
	 * Categories
	 *
	 * Creates a list of movie categories
	 *
	 * Usage:
	 * {{ movie:categories order-by="title" limit="5" }}
	...
	public function categories()
	{
		...
		foreach ($categories as &$category)
		{
			$category->url = site_url('movie/category/'.$category->slug);
		}
	 ...
	 * Tag/Keyword List
	 *
	 * Create a list of movie keywords/tags
	 *
	 * Usage:
	 * {{ movie:tags limit="10" }}
	...
	public function tags()
	{
		...
		$posts = $this->db->select('keywords')->get('movie')->result();
