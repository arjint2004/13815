...
class Admin_Categories extends Admin_Controller
{
	 ...
	 * - load the movie_categories model.
	 * - load the categories and movie language files.
	...
	public function index()
	{
		...
		$total_rows = $this->movie_categories_m->count_all();
		$pagination = create_pagination('admin/movie/categories/index', $total_rows, Settings::get('records_per_page'), 5);

		// Using this data, get the relevant results
		$categories = $this->movie_categories_m
								->order_by('title')
	...
	public function create()
	{
		...
		{
			if ($id = $this->movie_categories_m->insert($this->input->post()))
			{
				// Fire an event. A new movie category has been created.
				Events::trigger('movie_category_created', $id);
			...
			redirect('admin/movie/categories');
		}
	...
	{
		// Get the category
		$category = $this->movie_categories_m->get($id);

		// ID specified?
		$category or redirect('admin/movie/categories/index');
		...
		if ($this->form_validation->run())
		{
			$this->movie_categories_m->update($id, $this->input->post())
			...
			// Fire an event. A movie category is being updated.
			Events::trigger('movie_category_updated', $id);

			redirect('admin/movie/categories/index');
		}
	...
	public function delete($id = 0)
	{
		...
		if (!empty($id_array))
		{
			...
			{
				if ($this->movie_categories_m->delete($id))
				{
					$deleted++;
					$deleted_ids[] = $id;
				}
			...
			Events::trigger('movie_category_deleted', $deleted_ids);
		}
		else
		{
	...
	{
		if ($this->movie_categories_m->check_title($title, $this->input->post('id')))
		{
	...
	{
		if ($this->movie_categories_m->check_slug($slug, $this->input->post('id')))
		{
	...
	public function create_ajax()
	{
		...
		if ($this->form_validation->run())
		{
			$id = $this->movie_categories_m->insert_ajax($this->input->post());
