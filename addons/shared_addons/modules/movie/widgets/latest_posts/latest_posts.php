...
/**
 * Show Latest movie in your site with a widget.
 *
...
class Widget_Latest_posts extends Widgets
{
	...
	public $title = array(
		'en' => 'Latest posts',
		'br' => 'Artigos recentes do Movie',
            'fa' => 'آخرین ارسال ها',
		'pt' => 'Artigos recentes do Movie',
	...
	public $description = array(
		'en' => 'Display latest movie posts with a widget',
		'br' => 'Mostra uma lista de navegação para abrir os últimos artigos publicados no Movie',
            'fa' => 'نمایش آخرین پست های وبلاگ در یک ویجت',
		'pt' => 'Mostra uma lista de navegação para abrir os últimos artigos publicados no Movie',
		'el' => 'Προβάλει τις πιο πρόσφατες αναρτήσεις στο ιστολόγιό σας',
		'fr' => 'Permet d\'afficher la liste des derniers posts du movie dans un Widget',
		'ru' => 'Выводит список последних записей блога внутри виджета',
		'id' => 'Menampilkan posting movie terbaru menggunakan widget',
	);
	...
	{
		// load the movie module's model
		class_exists('Movie_m') OR $this->load->model('movie/movie_m');

		// sets default number of posts to be shown
		$options['limit'] = ( ! empty($options['limit'])) ? $options['limit'] : 5;

		// retrieve the records using the movie module's model
		$movie_widget = $this->movie_m
			->limit($options['limit'])
