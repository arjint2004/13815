 ...
 * @package  PyroCMS\Core\Modules\Movieslider\Plugins
 */
class Plugin_Movieslider extends Plugin
{
	...
	public $description = array(
		'en' => 'A plugin to display information such as movieslider categories and posts.',
		'br' => 'Um plugin para exibir informações como categorias e postagens do movieslider.',
            'fa' => 'یک پلاگین برای نمایش اطلاعاتی مانند مجموعه های بلاگ و پست ها',
        'fr' => 'Un plugin permettant d\'afficher des informations comme les catégories et articles du movieslider.'
	);
		...
		$info = array(
			'posts' => array(
				'description' => array(// a single sentence to explain the purpose of this method
					'en' => 'Display movieslider posts optionally filtering them by category.',
					'br' => 'Mostra postagens do movieslider opcionalmente filtrados por categoria.',
                    'fr' => 'Permet d\'afficher des articles de movieslider en les filtrants par catégorie.'
				),
			...
			'categories' => array(
				'description' => array(
					'en' => 'List movieslider categories.',
					'br' => 'Lista categorias do movieslider.',
                    'fr' => 'Lister les catégories du movieslider'
				),
				'single' => false,
				'double' => true,
			...
			'count_posts' => array(
				'description' => array(
					'en' => 'Count movieslider posts that meet the conditions specified.',
					'br' => 'Conta as postagens do movieslider que satisfazem as condições especificadas.',
                    'fr' => 'Permet de compter les articles de movieslider qui remplissent certaines conditions spécifiées.'
				),
				'single' => true,
				'double' => false,
				'variables' => '',
			...
			'tags' => array(
				'description' => array(
					'en' => 'Retrieve all tags that have been applied to movieslider posts.',
					'br' => 'Recupera todas as tags que foram aplicadas à postagens do movieslider.',
	 ...
	 * Movieslider List
	 *
	 * Creates a list of movieslider posts. Takes all of the parameters
	 * available to streams, sans stream, where, and namespace.
	 *
	 * Usage:
	 * {{ movieslider:posts limit="5" }}
	 *		<h2>{{ title }}</h2>
	 * {{ /movieslider:posts }}
	 *
	 * @param	array
	...
	public function posts()
	{
		...
		if ($category_string = $this->attribute('category'))
		{
			...
			{
				$cate_filter_by[] = '`'.$this->db->dbprefix('movieslider_categories').'`.`'.(is_numeric($category) ? 'id' : 'slug').'` = \''.$category."'";
			}
		...
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movieslider_categories.title', true)." as 'category_title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movieslider_categories.slug', true)." as 'category_slug'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movieslider_categories.title', true)." as 'category||title'";
		$this->row_m->sql['select'][] = $this->db->protect_identifiers('movieslider_categories.slug', true)." as 'category||slug'";
		$this->row_m->sql['join'][] = 'LEFT JOIN '.$this->db->protect_identifiers('movieslider_categories', true).' ON '.$this->db->protect_identifiers('movieslider_categories.id', true).' = '.$this->db->protect_identifiers('movieslider.category_id', true);
		...
		// Return our content.	
		return $this->streams->parse->parse_tag_content($this->content(), $posts, 'movieslider', 'moviesliders', $loop);
	}

	/**
	 * Categories
	 *
	 * Creates a list of movieslider categories
	 *
	 * Usage:
	 * {{ movieslider:categories order-by="title" limit="5" }}
	 *		<a href="{{ url }}" class="{{ slug }}">{{ title }}</a>
	 * {{ /movieslider:categories }}
	 *
	 * @param array
	 * @return array
	 */
	public function categories()
	{
		...
		$categories = $this->db
			...
			->limit($limit)
			->get('movieslider_categories')
			->result();

		foreach ($categories as &$category)
		{
			$category->url = site_url('movieslider/category/'.$category->slug);
		}
	 ...
	 *
	 * Usage:
	 * {{ movieslider:count_posts author_id="1" }}
	 *
	...
	public function count_posts()
	{
		...
		return $this->db->count_all_results('movieslider');
	}
	
	/**
	 * Tag/Keyword List
	 *
	 * Create a list of movieslider keywords/tags
	 *
	 * Usage:
	 * {{ movieslider:tags limit="10" }}
	...
	public function tags()
	{
		...
		$posts = $this->db->select('keywords')->get('movieslider')->result();
		...
		foreach($posts as $p)
		{
			...
			foreach($kw as $k)
			{
				...
				if(!in_array($k, $buffer)) // let's force a unique list
				{
					...
					$tags[] = array(
						'title' => ucfirst($k),
						'url'   => site_url('movieslider/tagged/'.$k)
					);
				}
			}
		}
