...
class Rss extends Public_Controller
{
	...
	{
		$posts = $this->pyrocache->model('movieslider_m', 'get_many_by', array(array(
			'status' => 'live',
			'limit' => Settings::get('rss_feed_items'))
		), Settings::get('rss_cache'));

		$this->output->set_content_type('application/rss+xml');
		echo $this->load->view('rss', $this->_build_feed($posts, $this->lang->line('movieslider:rss_name_suffix')), true);
	}
	...
	public function category($slug = '')
	{
		...
		if ( ! $category = $this->movieslider_categories_m->get_by('slug', $slug))
		{
			redirect('movieslider/rss/all.rss');
		}
		...
		$this->output->set_content_type('application/rss+xml');
		echo $this->load->view('rss', $this->_build_feed($posts, $category->title.$this->lang->line('movieslider:rss_category_suffix')), true);
	}
	...
	public function _build_feed($posts = array(), $suffix = '')
	{
		...
		$data->rss->page_description = sprintf($this->lang->line('movieslider:rss_posts_title'), Settings::get('site_name'));
		...
		if ( ! empty($posts))
		{
			foreach ($posts as $row)
			{
				...
				$row->link = site_url('movieslider/'.date('Y/m', $row->created_on).'/'.$row->slug);
